...
public class PupilManager : MonoBehaviour 
{
	...
	{	
		...
		GameObject gazePositionObject = GameObject.Find("GazePositions");
		gazePositions = gazePositionObject.GetComponent<Text>();
	...
	void Update()
	{
		...
		if (PupilTools.IsGazing)
		{
			if (PupilTools.CalibrationMode == Calibration.Mode._2D)
			{
				...
				gazePositions.text = PupilData._2D.GazePosition.ToString();				
			}
			else if (PupilTools.CalibrationMode == Calibration.Mode._3D)
			{
				// Debug.Log(PupilData._3D.GazePosition);
				gazePositions.text = PupilData._3D.GazePosition.ToString();
			}
		} 
